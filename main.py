...
def get_start(message):
    ...
    if get_language(message.chat.id, session_handler) == "Tajik":
        if message.text == "Кӯмак бо қарз":           
            ...
            # [markup.add(types.KeyboardButton(i)) for i in currencies]
            ...
            item3 = types.KeyboardButton("Бонки Байналмилалии Точикистон")
            markup.add(item1)
            markup.add(item2)
            markup.add(item3)
        ...
        if message.text == "Помощь с кредитом":
            ...
            item3 = types.KeyboardButton("Международный Банк Таджикистана")
            markup.add(item1)
            markup.add(item2)
            markup.add(item3)
